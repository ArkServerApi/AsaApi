...
struct FMath : public FPlatformMath
{
	 ...
	 */
	UE_NODISCARD static FVector VRandCone(FVector const& Dir, float ConeHalfAngleRad);
	 ...
	 */
	UE_NODISCARD static FVector VRandCone(FVector const& Dir, float HorizontalConeHalfAngleRad, float VerticalConeHalfAngleRad);

	/** Returns a random point, uniformly distributed, within the specified radius */
	UE_NODISCARD static FVector2D RandPointInCircle(float CircleRadius);

	/** Returns a random point within the passed in bounding box */
	UE_NODISCARD static FVector RandPointInBox(const FBox& Box);
	 ...
	 */
	UE_NODISCARD static FVector GetReflectionVector(const FVector& Direction, const FVector& SurfaceNormal);
...
#undef FASTASIN_HALF_PI
	 ...
	 */
	UE_NODISCARD static float FixedTurn(float InCurrent, float InDesired, float InDeltaRate);
	 ...
	 */
	UE_NODISCARD static FVector2D GetAzimuthAndElevation(const FVector &Direction, const FVector &AxisX, const FVector &AxisY, const FVector &AxisZ);
	...
	UE_NODISCARD static FVector VInterpNormalRotationTo(const FVector& Current, const FVector& Target, float DeltaTime, float RotationSpeedDegrees);

	/** Interpolate vector from Current to Target with constant step */
	UE_NODISCARD static FVector VInterpConstantTo(const FVector& Current, const FVector& Target, float DeltaTime, float InterpSpeed);

	/** Interpolate vector from Current to Target. Scaled by distance to Target, so it has a strong start speed and ease out. */
	UE_NODISCARD static FVector VInterpTo( const FVector& Current, const FVector& Target, float DeltaTime, float InterpSpeed );
	
	/** Interpolate vector2D from Current to Target with constant step */
	UE_NODISCARD static FVector2D Vector2DInterpConstantTo( const FVector2D& Current, const FVector2D& Target, float DeltaTime, float InterpSpeed );

	/** Interpolate vector2D from Current to Target. Scaled by distance to Target, so it has a strong start speed and ease out. */
	UE_NODISCARD static FVector2D Vector2DInterpTo( const FVector2D& Current, const FVector2D& Target, float DeltaTime, float InterpSpeed );

	/** Interpolate rotator from Current to Target with constant step */
	UE_NODISCARD static FRotator RInterpConstantTo( const FRotator& Current, const FRotator& Target, float DeltaTime, float InterpSpeed);

	/** Interpolate rotator from Current to Target. Scaled by distance to Target, so it has a strong start speed and ease out. */
	UE_NODISCARD static FRotator RInterpTo( const FRotator& Current, const FRotator& Target, float DeltaTime, float InterpSpeed);
	...
	UE_NODISCARD static FLinearColor CInterpTo(const FLinearColor& Current, const FLinearColor& Target, float DeltaTime, float InterpSpeed);

	/** Interpolate quaternion from Current to Target with constant step (in radians) */
	template< class T >
	UE_NODISCARD static UE::Math::TQuat<T> QInterpConstantTo(const UE::Math::TQuat<T>& Current, const UE::Math::TQuat<T>& Target, float DeltaTime, float InterpSpeed);

	/** Interpolate quaternion from Current to Target. Scaled by angle to Target, so it has a strong start speed and ease out. */
	template< class T >
	UE_NODISCARD static UE::Math::TQuat<T> QInterpTo(const UE::Math::TQuat<T>& Current, const UE::Math::TQuat<T>& Target, float DeltaTime, float InterpSpeed);
	...
	*/
	...
	UE_NODISCARD static uint32 ComputeProjectedSphereScissorRect(FIntRect& InOutScissorRect, FVector SphereOrigin, float Radius, FVector ViewOrigin, const FMatrix& ViewMatrix, const FMatrix& ProjMatrix);
	 ...
	 */
	UE_NODISCARD static bool PlaneAABBIntersection(const FPlane& P, const FBox& AABB);
	 ...
	 */
	UE_NODISCARD static int32 PlaneAABBRelativePosition(const FPlane& P, const FBox& AABB);
	 ...
	 */
	UE_NODISCARD static bool SphereConeIntersection(const FVector& SphereCenter, float SphereRadius, const FVector& ConeAxis, float ConeAngleSin, float ConeAngleCos);

	/** Find the point on the line segment from LineStart to LineEnd which is closest to Point */
	UE_NODISCARD static FVector ClosestPointOnLine(const FVector& LineStart, const FVector& LineEnd, const FVector& Point);

	/** Find the point on the infinite line between two points (LineStart, LineEnd) which is closest to Point */
	UE_NODISCARD static FVector ClosestPointOnInfiniteLine(const FVector& LineStart, const FVector& LineEnd, const FVector& Point);
	...
	UE_NODISCARD static float PointDistToLine(const FVector &Point, const FVector &Direction, const FVector &Origin);
	 ...
	 */
	UE_NODISCARD static FVector ClosestPointOnSegment(const FVector &Point, const FVector &StartPoint, const FVector &EndPoint);
	...
	* @return	point on the segment defined by (StartPoint, EndPoint) that is closest to Point.
	*/
	UE_NODISCARD static FVector2D ClosestPointOnSegment2D(const FVector2D &Point, const FVector2D &StartPoint, const FVector2D &EndPoint);
	 ...
	 */
	UE_NODISCARD static float PointDistToSegment(const FVector &Point, const FVector &StartPoint, const FVector &EndPoint);
	 ...
	 */
	UE_NODISCARD static float PointDistToSegmentSquared(const FVector &Point, const FVector &StartPoint, const FVector &EndPoint);
	 ...
	 */
	UE_NODISCARD static float GetTForSegmentPlaneIntersect(const FVector& StartPoint, const FVector& EndPoint, const FPlane& Plane);
	...
	*/
	 ...
	 */
	UE_NODISCARD static FVector ClosestPointOnTriangleToPoint(const FVector& Point, const FVector& A, const FVector& B, const FVector& C);
	 ...
	 */
	UE_NODISCARD static FVector ClosestPointOnTetrahedronToPoint(const FVector& Point, const FVector& A, const FVector& B, const FVector& C, const FVector& D);
	 ...
	 */
	UE_NODISCARD static bool GetDistanceWithinConeSegment(FVector Point, FVector ConeStartPoint, FVector ConeLine, float RadiusAtStart, float RadiusAtEnd, float &PercentageOut);
	 ...
	 */
	UE_NODISCARD static bool PointsAreCoplanar(const TArray<FVector>& Points, const float Tolerance = 0.1f);
	 ...
	 */
	UE_NODISCARD static float TruncateToHalfIfClose(float F, float Tolerance = UE_SMALL_NUMBER);
	UE_NODISCARD static double TruncateToHalfIfClose(double F, double Tolerance = UE_SMALL_NUMBER);
	...
	*/
	UE_NODISCARD static float RoundHalfToEven(float F);
	UE_NODISCARD static double RoundHalfToEven(double F);
	...
	* @return		The rounded integer
	*/
	UE_NODISCARD static float RoundHalfFromZero(float F);
	UE_NODISCARD static double RoundHalfFromZero(double F);
	...
	* @return		The rounded integer
	*/
	UE_NODISCARD static float RoundHalfToZero(float F);
	UE_NODISCARD static double RoundHalfToZero(double F);
	...
	*/
	 ...
	 */
	UE_NODISCARD static FString FormatIntToHumanReadable(int32 Val);
	 ...
	 */
	UE_NODISCARD static bool MemoryTest( void* BaseAddress, uint32 NumBytes );
	 ...
	 */
	UE_NODISCARD static FVector GetBaryCentric2D(const FVector& Point, const FVector& A, const FVector& B, const FVector& C);
	 ...
	 *							                                or Point = A + b*(B-A) + c*(C-A) = (1-b-c)*A + b*B + c*C
	 */
	UE_NODISCARD static FVector GetBaryCentric2D(const FVector2D& Point, const FVector2D& A, const FVector2D& B, const FVector2D& C);
	 ...
	 *							                               or Point = A + b*(B-A) + c*(C-A) = (1-b-c)*A + b*B + c*C
	 */
	UE_NODISCARD static FVector ComputeBaryCentric2D(const FVector& Point, const FVector& A, const FVector& B, const FVector& C);
	 ...
	 */
	UE_NODISCARD static FVector4 ComputeBaryCentric3D(const FVector& Point, const FVector& A, const FVector& B, const FVector& C, const FVector& D);
	 ...
	 */
	UE_NODISCARD static float PerlinNoise1D(float Value);
	...
	* @return	Perlin noise in the range of -1.0 to 1.0
	*/
	UE_NODISCARD static float PerlinNoise2D(const FVector2D& Location);
	...
	* @return	Perlin noise in the range of -1.0 to 1.0
	*/
	UE_NODISCARD static float PerlinNoise3D(const FVector& Location);
